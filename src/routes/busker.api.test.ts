...
//         const enrollResult = await request(app).post(prefixApiPath + apiPath.enrollBusker).set("Cookie", [cookies]).send({ ...mockRequestData.generateSendData({ ...enrollBuskerData }) })
//         expect(enrollResult.statusCode).toBe(200);
//     });
//     it(" it should return status 200 if  repeat enroll", async () => {
//         const enrollResult = await request(app).post(prefixApiPath + apiPath.enrollBusker).set("Cookie", [cookies]).send({ ...mockRequestData.generateSendData({ ...enrollBuskerData }) })
//         expect(enrollResult.statusCode).toBe(200);
//         const enrollResult1 = await request(app).post(prefixApiPath + apiPath.enrollBusker).set("Cookie", [cookies]).send({ ...mockRequestData.generateSendData({ ...enrollBuskerData }) })
//         expect(enrollResult1.statusCode).toBe(401)
//     });
//     it(" it should return status 401 if use incorrect parameter", async () => {
//         const enrollResult1 = await request(app).post(prefixApiPath + apiPath.enrollBusker).set("Cookie", [cookies]).send({ ...mockRequestData.generateSendData({}) })
...
//     let performanceData
//     beforeEach(async () => {
//         const enrollBuskerResult = await request(app).post(prefixApiPath + apiPath.enrollBusker).set("Cookie", [cookies]).send({ ...mockRequestData.generateSendData({ ...enrollBuskerData }) })
...
//         const result = await request(app).post(prefixApiPath + apiPath.performance).set("Cookie", [cookies]).send({ ...mockRequestData.generateSendData({ ...performanceData }) })
//         expect(result.statusCode).toBe(200);
//     });
//     it(" it should return status 400 if incorrect apply", async () => {
//         const result = await request(app).post(prefixApiPath + apiPath.performance).set("Cookie", [cookies]).send({ ...mockRequestData.generateSendData({}) })
...
//         performanceData = buskerRepo.generateDiffPerformanceData(memberId, buskerRepo.getCurrentDate())
//         const applyResult = await request(app).post(prefixApiPath + apiPath.performance)
